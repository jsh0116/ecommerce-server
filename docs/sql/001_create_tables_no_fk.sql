-- ============================================
-- HHPLUS E-Commerce Database Schema (No Foreign Keys)
-- 마이크로서비스 아키텍처용 (FK 제거 버전)
-- MySQL 8.0+ / MariaDB 10.5+
-- ============================================
-- 이 스크립트는 MySQL Workbench에서 직접 실행 가능합니다.
-- 주의: 외래키가 없으므로 데이터 무결성은 애플리케이션에서 관리해야 합니다.
...
CREATE TABLE IF NOT EXISTS product_variants (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '변량 ID',
...
CREATE TABLE IF NOT EXISTS cart_items (
    ...
    variant_id BIGINT NOT NULL COMMENT '상품 변량 ID',
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    ) NOT NULL DEFAULT 'PENDING_PAYMENT' COMMENT '주문 상태',
    ...
    discount_amount BIGINT NOT NULL DEFAULT 0 COMMENT '할인액',
    final_amount BIGINT NOT NULL COMMENT '최종금액',
    request_message VARCHAR(255) NULL COMMENT '배송 요청사항',
    reservation_expires_at TIMESTAMP NULL COMMENT '재고 예약 만료',
...
CREATE TABLE IF NOT EXISTS payment_failures (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '실패 기록 ID',
    order_id BIGINT NOT NULL COMMENT '주문 ID',
...
CREATE TABLE IF NOT EXISTS webhook_logs (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '웹훅 로그 ID',
...
CREATE TABLE IF NOT EXISTS webhook_retry_queue (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '재시도 큐 ID',
...
-- 외래키가 제거되었으므로 데이터 무결성은 애플리케이션에서 관리하세요.
