...
- **스캔 범위 축소: 200배 (100,000 / 500)**
- **배치 효율: 1,000배+ (1,000번 UPDATE → 1번 UPDATE)**
...
| 쿼리 | 테이블 | 최적화 전 (행) | 최적화 후 (행) | 스캔 범위 축소 | 주요 인덱스 |
|------|--------|---------------|---------------|------------|-----------|
...
**주의**: 위 개선율은 스캔 범위 축소입니다.
실제 응답 시간 개선은 데이터 크기에 따라 5배~200배+ 범위입니다.
...
✅ **복합 인덱스 도입으로:**
- Orders 쿼리: **20,000배 스캔 범위 축소** (실제 응답: 현재 5-20배, 장기 200배+)
- Reservations 쿼리: **200배 스캔 범위 축소** (실제 응답: 현재 5배, 장기 200배+)
- 배치 작업: **O(N) → O(1) 최적화** (1,000배+ 효율화)
