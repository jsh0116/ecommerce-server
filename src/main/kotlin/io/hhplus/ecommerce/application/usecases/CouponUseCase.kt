 ...
 * 쿠폰 유스케이스
 *
 * CouponLockService를 통해 분산 락을 이용하여 멀티 서버 환경에서도 안전한 동시성 제어를 구현합니다.
 * - 쿠폰 ID별로 분산 락을 적용하여 race condition 방지
 * - 3초 대기 후 락 획득 실패 시 예외 발생
 * - 멀티 서버 환경에서도 정확한 선착순 처리 보장
 * - 테스트 환경에서는 Mock CouponLockService를 주입받음
 */
@Service
class CouponUseCase(
     ...
     * 선착순 쿠폰 발급 (분산 락 적용)
     *
     * CouponLockService를 통해 멀티 서버 환경에서도 안전한 race condition 방지:
